        ...
        COALESCE((
          SELECT JSON_ARRAYAGG(
            JSON_OBJECT(
              ...
              'bon_commande_id', si.bon_commande_id,
              ...
              'prix_achat', COALESCE(
                ...
                (
                  ...
                  WHERE ci2.bon_commande_id = si.bon_commande_id
                    AND ci2.product_id = si.product_id
...
router.post('/', forbidRoles('ChefChauffeur'), async (req, res) => {
  ...
  if (!date_creation || !montant_total || !created_by) {
    ...
    for (const it of items) {
      ...
      // Insert first to get item id (needed for FIFO allocations)
      ...
      const sortieItemId = ins.insertId;
      if (shouldAllocate) {
        const alloc = await consumeFifo(connection, {
          ...
          target_table: 'sortie_items',
          target_item_id: sortieItemId,
        });
          ...
          await connection.execute(
            `UPDATE sortie_items
                SET bon_commande_id = ?,
                    prix_achat_snapshot = ?
              WHERE id = ?`,
            [useSingleBon ? alloc.single_bon_commande_id : null, alloc.avg_cost ?? null, sortieItemId]
          );
        }
      }
    }
...
router.put('/:id', async (req, res) => {
  ...
  adresse_livraison,
    ...
    if (isChefChauffeur) {
      ...
      // Lock remises
      req.body.remise_is_client = oldBon.remise_is_client;
      req.body.remise_id = oldBon.remise_id;
      // Lock livraisons
    ...
    if (fifo && oldStatut !== 'Annulé') {
      const [oldItemIds] = await connection.execute('SELECT id FROM sortie_items WHERE bon_sortie_id = ? ORDER BY id ASC', [id]);
      for (const r of oldItemIds || []) {
        await restoreAllocationsForTarget(connection, 'sortie_items', r.id);
      }
    }
...
router.patch('/:id/statut', verifyToken, async (req, res) => {
  ...
  try {
    ...
    const [oldRows] = await connection.execute(
      'SELECT statut FROM bons_sortie WHERE id = ? FOR UPDATE',
      [id]
    );
    ...
    // Stock: Sortie => effet = -quantite quand pas Annulé
    ...
    if (enteringCancelled || leavingCancelled) {
      ...
      if (fifo) {
        const [rowsItems] = await connection.execute(
          'SELECT id, product_id, variant_id, quantite FROM sortie_items WHERE bon_sortie_id = ? ORDER BY id ASC',
          [id]
        );
        if (enteringCancelled) {
          for (const it of rowsItems || []) {
            await restoreAllocationsForTarget(connection, 'sortie_items', it.id);
          }
        } else {
          for (const it of rowsItems || []) {
            const alloc = await consumeFifo(connection, {
...
router.post('/:id/mark-avoir', async (req, res) => {
  ...
  const today = new Date().toISOString().split('T')[0];
    ...
    // FIFO: restoring stock from a sortie also restores the FIFO layers.
    const fifo = await isFifoEnabled(connection);
    if (fifo) {
      for (const it of items) {
        await restoreAllocationsForTarget(connection, 'sortie_items', it.id);
