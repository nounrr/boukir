  ...
  try {
    ...
    const [rows] = await pool.query(
      `SELECT
         ...
         COALESCE(ps.prix_achat, pv.prix_achat, p.prix_achat) AS prix_achat,
         COALESCE(ps.cout_revient, pv.cout_revient, p.cout_revient) AS cout_revient,
      ...
      [rows] = await pool.query(
        `SELECT * FROM (
           ...
           SELECT
             ...
             NULL AS bon_commande_id,
             NULL AS snapshot_created_at,
             COALESCE(pv.prix_achat, p.prix_achat) AS prix_achat,
