...
// S'assure que les colonnes inclus_en_caisse / inclus_en_caisse_at existent sur avoirs_client
async function ensureInclusEnCaisseColumn() {
  try {
    const [cols] = await pool.query(
      "SHOW COLUMNS FROM avoirs_client LIKE 'inclus_en_caisse'"
    );
    if (!Array.isArray(cols) || cols.length === 0) {
      await pool.query(
        "ALTER TABLE avoirs_client ADD COLUMN inclus_en_caisse TINYINT(1) NOT NULL DEFAULT 0 AFTER statut"
      );
    }
    const [colsAt] = await pool.query(
      "SHOW COLUMNS FROM avoirs_client LIKE 'inclus_en_caisse_at'"
    );
    if (!Array.isArray(colsAt) || colsAt.length === 0) {
      await pool.query(
        "ALTER TABLE avoirs_client ADD COLUMN inclus_en_caisse_at DATETIME NULL AFTER inclus_en_caisse"
      );
    }
  } catch (e) {
    console.error('ensureInclusEnCaisseColumn avoirs_client:', e?.sqlMessage || e?.message || e);
  }
}
...
/* =================== DELETE /:id =================== */
// PATCH /avoirs_client/:id/inclus-en-caisse - Basculer l'inclusion en caisse
router.patch('/:id/inclus-en-caisse', verifyToken, async (req, res) => {
  try {
    ...
    console.error('PATCH /avoirs_client/:id/inclus-en-caisse error:', error);
    res.status(500).json({ message: 'Erreur du serveur', error: error?.sqlMessage || error?.message });
  }
});
