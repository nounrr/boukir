-- ============================================================
-- FIX UPDATE - Tous les produits eligibles
-- LOGIQUE FIFO: vider chaque snapshot completement (du plus
-- ancien au plus recent) avant de passer au suivant.
--
-- CONDITIONS:
--   - item.product_snapshot_id IS NULL
--   - date_creation >= '2026-03-01'
--   - products.prix_achat = 0 AND products.cout_revient = 0
--   - il existe au moins un product_snapshot exact pour product_id/variant_id
--
-- REGLES:
...
--   - avoir_client/avoir_comptant => AJOUTER
--
-- IMPORTANT: ARRETER LE SERVEUR NODE (npm run dev:full)
...
WHERE EXISTS (
    ...
    WHERE p.id = ps.product_id
      AND COALESCE(p.prix_achat, 0) = 0
      AND COALESCE(p.cout_revient, 0) = 0
)
...
WHERE EXISTS (
    SELECT 1
    FROM products p
    WHERE p.id = product_snapshot.product_id
      AND COALESCE(p.prix_achat, 0) = 0
      AND COALESCE(p.cout_revient, 0) = 0
)
...
DROP TEMPORARY TABLE IF EXISTS fix_items_all;
DROP TEMPORARY TABLE IF EXISTS fix_mapping_all;
DROP TEMPORARY TABLE IF EXISTS snapshot_stock_all;

CREATE TEMPORARY TABLE fix_items_all (
...
INSERT INTO fix_items_all (product_id, variant_id, bon_type, item_id, item_qte)
...
UNION ALL
SELECT acpi.product_id, acpi.variant_id, 'avoir_comptant', acpi.id, acpi.quantite
...
CREATE TEMPORARY TABLE fix_mapping_all (
    product_id INT,
    variant_id INT NULL,
    bon_type VARCHAR(20),
    item_id INT,
...
CREATE TEMPORARY TABLE snapshot_stock_all AS
SELECT DISTINCT
    ps.product_id,
    ps.variant_id,
    ps.id AS snapshot_id,
    ps.quantite AS remaining,
    ps.prix_vente,
    ps.created_at
FROM product_snapshot ps
JOIN fix_items_all fi ON fi.product_id = ps.product_id
    AND ((NULLIF(fi.variant_id, 0) IS NULL AND ps.variant_id IS NULL) OR ps.variant_id = NULLIF(fi.variant_id, 0));

ALTER TABLE snapshot_stock_all ADD INDEX idx_snap (snapshot_id);
ALTER TABLE snapshot_stock_all ADD INDEX idx_product_variant_remaining (product_id, variant_id, remaining);

SELECT '=== ITEMS A TRAITER ===' AS etape;
SELECT product_id, variant_id, bon_type, COUNT(*) AS nb_items, SUM(item_qte) AS total_qte
FROM fix_items_all
GROUP BY product_id, variant_id, bon_type
ORDER BY product_id, variant_id, bon_type;
...
DROP PROCEDURE IF EXISTS distribute_fix_all_products;

DELIMITER //
CREATE PROCEDURE distribute_fix_all_products()
BEGIN
    ...
    -- 1. AVOIRS (retours => ajouter au snapshot exact le plus recent)
    avoir_loop: LOOP
        SELECT COUNT(*) INTO v_count FROM fix_items_all WHERE bon_type IN ('avoir_client', 'avoir_comptant');
        IF v_count = 0 THEN LEAVE avoir_loop; END IF;

        SELECT id, product_id, variant_id, bon_type, item_id, item_qte
        INTO v_item_row_id, v_product_id, v_variant_id, v_bon_type, v_item_id, v_item_qte
        FROM fix_items_all
        WHERE bon_type IN ('avoir_client', 'avoir_comptant')
        ORDER BY product_id, variant_id, id
        LIMIT 1;

        SET v_snap_id = NULL;
        SELECT COUNT(*) INTO v_count
        FROM snapshot_stock_all
        ...
        IF v_snap_id IS NOT NULL THEN
            UPDATE snapshot_stock_all SET remaining = remaining + v_item_qte WHERE snapshot_id = v_snap_id;
        END IF;

        INSERT INTO fix_mapping_all VALUES (v_product_id, v_variant_id, v_bon_type, v_item_id, v_item_qte, v_snap_id);
        DELETE FROM fix_items_all WHERE id = v_item_row_id;
    END LOOP;

    -- 2. COMPTANT + SORTIE: FIFO par produit/variante - vider chaque snapshot completement
    snap_loop: LOOP
        SELECT COUNT(*) INTO v_count FROM fix_items_all;
        ...
        SELECT COUNT(*) INTO v_count
        FROM fix_items_all fi
        JOIN snapshot_stock_all ss ON ss.product_id = fi.product_id
            AND ((NULLIF(fi.variant_id, 0) IS NULL AND ss.variant_id IS NULL) OR ss.variant_id = NULLIF(fi.variant_id, 0))
            AND ss.remaining > 0;

        IF v_count = 0 THEN
            INSERT INTO fix_mapping_all (product_id, variant_id, bon_type, item_id, item_qte, target_snapshot_id)
            SELECT product_id, variant_id, bon_type, item_id, item_qte, NULL FROM fix_items_all;
            DELETE FROM fix_items_all;
            LEAVE snap_loop;
        END IF;
        ...
        ORDER BY MIN(ss.created_at) ASC, fi.product_id, fi.variant_id
        LIMIT 1;
        ...
        drain_loop: LOOP
            SELECT remaining INTO v_snap_remaining FROM snapshot_stock_all WHERE snapshot_id = v_snap_id;
            ...
            ELSE
                ...
                WHERE product_id = v_product_id
                  AND ((NULLIF(v_variant_id, 0) IS NULL AND variant_id IS NULL) OR variant_id = NULLIF(v_variant_id, 0))
                ORDER BY item_qte ASC, id ASC
                LIMIT 1;
            END IF;

            IF v_item_row_id IS NULL THEN LEAVE drain_loop; END IF;

            UPDATE snapshot_stock_all
...
CALL distribute_fix_all_products();
DROP PROCEDURE IF EXISTS distribute_fix_all_products;
...
SELECT
    ...
    ps.created_at
FROM product_snapshot ps
JOIN snapshot_stock_all ss ON ss.snapshot_id = ps.id
LEFT JOIN (
    SELECT target_snapshot_id,
        SUM(CASE WHEN bon_type IN ('comptant','sortie') THEN item_qte ELSE 0 END) AS a_deduire,
        SUM(CASE WHEN bon_type IN ('avoir_client','avoir_comptant') THEN item_qte ELSE 0 END) AS a_ajouter
    FROM fix_mapping_all WHERE target_snapshot_id IS NOT NULL
    GROUP BY target_snapshot_id
) ch ON ch.target_snapshot_id = ps.id
ORDER BY ps.product_id, ps.variant_id, ps.created_at DESC;

SELECT '=== ITEMS SANS CIBLE AVANT FALLBACK ===' AS etape;
SELECT product_id, variant_id, bon_type, COUNT(*) AS items_sans_cible, SUM(item_qte) AS qte_sans_cible
FROM fix_mapping_all
...
JOIN (
    SELECT target_snapshot_id,
        ...
        SUM(CASE WHEN bon_type IN ('avoir_client','avoir_comptant') THEN item_qte ELSE 0 END) AS total_a_ajouter
    FROM fix_mapping_all WHERE target_snapshot_id IS NOT NULL
    GROUP BY target_snapshot_id
...
JOIN fix_mapping_all fm ON fm.item_id = ci.id AND fm.bon_type = 'comptant'
SET ci.product_snapshot_id = fm.target_snapshot_id,
    ci.is_indisponible = 0
WHERE fm.target_snapshot_id IS NOT NULL;

-- 4c: Lier sortie_items
UPDATE sortie_items si
JOIN fix_mapping_all fm ON fm.item_id = si.id AND fm.bon_type = 'sortie'
SET si.product_snapshot_id = fm.target_snapshot_id,
    si.is_indisponible = 0
WHERE fm.target_snapshot_id IS NOT NULL;

-- 4d: Lier avoir_client_items
UPDATE avoir_client_items aci
JOIN fix_mapping_all fm ON fm.item_id = aci.id AND fm.bon_type = 'avoir_client'
SET aci.product_snapshot_id = fm.target_snapshot_id
WHERE fm.target_snapshot_id IS NOT NULL;

-- 4e: Lier avoir_comptant_items
UPDATE avoir_comptant_items acpi
JOIN fix_mapping_all fm ON fm.item_id = acpi.id AND fm.bon_type = 'avoir_comptant'
SET acpi.product_snapshot_id = fm.target_snapshot_id
WHERE fm.target_snapshot_id IS NOT NULL;

-- 4f: Si aucun stock positif n'a permis la liaison, lier au dernier snapshot exact
...
JOIN (
    ...
    WHERE ci2.product_snapshot_id IS NULL
      ...
      AND COALESCE(p2.prix_achat, 0) = 0
      AND COALESCE(p2.cout_revient, 0) = 0
      AND EXISTS (
          SELECT 1 FROM product_snapshot ps
          WHERE ps.product_id = ci2.product_id
...
JOIN (
    ...
    WHERE si2.product_snapshot_id IS NULL
      ...
      AND EXISTS (
          SELECT 1 FROM product_snapshot ps
          WHERE ps.product_id = si2.product_id
...
FROM product_snapshot
WHERE EXISTS (
    SELECT 1 FROM snapshot_stock_all ss
    WHERE ss.product_id = product_snapshot.product_id
      AND ((NULLIF(ss.variant_id, 0) IS NULL AND product_snapshot.variant_id IS NULL) OR product_snapshot.variant_id = NULLIF(ss.variant_id, 0))
)
GROUP BY product_id, variant_id
...
UNION ALL
SELECT acpi.product_id, acpi.variant_id, 'avoir_comptant', COUNT(*)
FROM avoir_comptant_items acpi
JOIN avoirs_comptant acp ON acp.id = acpi.avoir_comptant_id
JOIN products p ON p.id = acpi.product_id
WHERE acpi.product_snapshot_id IS NULL
  ...
  AND COALESCE(p.prix_achat, 0) = 0
  AND COALESCE(p.cout_revient, 0) = 0
GROUP BY acpi.product_id, acpi.variant_id;
