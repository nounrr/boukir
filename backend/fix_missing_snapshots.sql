...
GROUP BY bc.id, bc.date_creation, bc.client_nom, ci.id, ci.product_id, p.designation, ci.quantite, ci.prix_unitaire
...
WHERE si.product_snapshot_id IS NULL
  ...
  AND p.prix_achat = 0
  AND p.cout_revient = 0
GROUP BY bs.id, bs.date_creation, si.id, si.product_id, p.designation, si.quantite, si.prix_unitaire
HAVING COUNT(ps.id) > 1
...
WHERE aci.product_snapshot_id IS NULL
  ...
  AND p.prix_achat = 0
  AND p.cout_revient = 0
GROUP BY ac.id, ac.date_creation, aci.id, aci.product_id, p.designation, aci.quantite, aci.prix_unitaire
HAVING COUNT(ps.id) > 1
...
WHERE acpi.product_snapshot_id IS NULL
  ...
  AND p.prix_achat = 0
  AND p.cout_revient = 0
GROUP BY acp.id, acp.date_creation, acp.client_nom, acpi.id, acpi.product_id, p.designation, acpi.quantite, acpi.prix_unitaire
HAVING COUNT(ps.id) > 1
...
JOIN (
    ...
    JOIN product_snapshot ps ON ps.product_id = ci2.product_id AND ps.quantite > 0
    WHERE ci2.product_snapshot_id IS NULL
      AND bc.date_creation > '2026-03-31'
      AND p2.prix_achat = 0
      AND p2.cout_revient = 0
    GROUP BY ci2.id, ci2.product_id, ci2.quantite
    HAVING COUNT(ps.id) > 1
...
JOIN (
    ...
    JOIN product_snapshot ps ON ps.product_id = si2.product_id AND ps.quantite > 0
    WHERE si2.product_snapshot_id IS NULL
      AND bs.date_creation > '2026-03-31'
      AND p2.prix_achat = 0
      AND p2.cout_revient = 0
    GROUP BY si2.id, si2.product_id, si2.quantite
    HAVING COUNT(ps.id) > 1
...
JOIN (
    ...
    JOIN product_snapshot ps ON ps.product_id = aci2.product_id AND ps.quantite > 0
    WHERE aci2.product_snapshot_id IS NULL
      AND ac.date_creation > '2026-03-31'
      AND p2.prix_achat = 0
      AND p2.cout_revient = 0
    GROUP BY aci2.id, aci2.product_id, aci2.quantite
    HAVING COUNT(ps.id) > 1
...
JOIN (
    ...
    JOIN product_snapshot ps ON ps.product_id = acpi2.product_id AND ps.quantite > 0
    WHERE acpi2.product_snapshot_id IS NULL
      AND acp.date_creation > '2026-03-31'
      AND p2.prix_achat = 0
      AND p2.cout_revient = 0
    GROUP BY acpi2.id, acpi2.product_id, acpi2.quantite
    HAVING COUNT(ps.id) > 1
