...
export type DeliveryRun = { id: number; chauffeur_id: number | null; vehicule_id: number; chauffeur_nom: string | null; vehicule_nom: string; status: string; started_at: string; ended_at: string | null; duration_minutes: number; notes?: string; items: DeliveryItem[] };
...
const deliveryApi = api.injectEndpoints({
  endpoints: b => ({
    ...
    startDelivery: b.mutation<{ id: number }, { chauffeur_id: number | null; vehicule_id: number; bons: Pick<DeliveryBon, 'bon_type' | 'bon_id'>[]; notes: string }>({ query: body => ({ url: '/delivery-runs/runs', method: 'POST', body }), invalidatesTags: ['Delivery', 'Bon'] }),
    ...
    deliveryStats: b.query<{ summary: Metrics; drivers: (Metrics & { id: number | null; name: string })[]; vehicles: (Metrics & { id: number; name: string })[]; daily: (Metrics & { day: string })[]; reasons: { reason: string; total: number }[]; waiting: number }, DeliveryFilters>({ query: params => ({ url: '/delivery-runs/stats', params }), providesTags: ['Delivery'] }),
  }),
});
