...
export const bonsApi = api.injectEndpoints({
        ...
        if (type === 'Ecommerce') {
          ...
          return orders.map((o: any) => ({
            ...
            items: (o.items || []).map((i: any) => ({
              ...
              unite: i.unite ?? i.unit_name ?? i.base_unit ?? '',
              unit_name: i.unit_name ?? i.unite ?? i.base_unit ?? '',
              base_unit: i.base_unit ?? '',
              prix_original: Number(i.prix_original || 0),
      ...
      query: ({ id, type, ...bonData }) => {
        ...
        switch (type) {
          ...
          case 'Charge':
            ...
            break;
          case 'AvoirCharge':
            endpoint = `/charges/${id}?type=avoir`;
            break;
          case 'Devis':
            endpoint = `/devis/${id}`;
            break;
  ...
  updateBonStatus: builder.mutation<Bon, { id: number; statut: string; type?: AnyBonType; force_clamp_percentages?: boolean }>({
      query: ({ id, statut, type, force_clamp_percentages }) => {
        ...
        switch (type) {
          ...
          case 'Charge':
            ...
            break;
          case 'AvoirCharge':
            endpoint = `/charges/${id}/statut?type=avoir`;
            break;
          case 'Devis':
    ...
    updateChargeInclusEnCaisse: builder.mutation<Bon, { id: number; inclus_en_caisse: boolean | number }>({
      query: ({ id, inclus_en_caisse }) => ({
        url: `/charges/${id}/inclus-en-caisse`,
        method: 'PATCH',
