...
const StatsDetailPage: React.FC = () => {
  ...
  const skipStatsData = !isPasswordVerified;
  const { data: products = [], isLoading: productsLoading } = useGetProductsQuery(undefined, { skip: skipStatsData });
  const { data: clients = [], isLoading: clientsLoading } = useGetAllClientsQuery(undefined, { skip: skipStatsData });
  const { data: bonsSortie = [], isLoading: bonsSortieLoading } = useGetBonsByTypeQuery('Sortie', { skip: skipStatsData });
  const { data: bonsComptant = [], isLoading: bonsComptantLoading } = useGetBonsByTypeQuery('Comptant', { skip: skipStatsData });
  const { data: bonsEcommerce = [], isLoading: bonsEcommerceLoading } = useGetBonsByTypeQuery('Ecommerce', { skip: skipStatsData });
  const { data: bonsCommandes = [], isLoading: bonsCommandesLoading } = useGetBonsByTypeQuery('Commande', { skip: skipStatsData });
  const { data: avoirsClient = [], isLoading: avoirsClientLoading } = useGetBonsByTypeQuery('Avoir', { skip: skipStatsData });
  const { data: avoirsFournisseur = [], isLoading: avoirsFournisseurLoading } = useGetBonsByTypeQuery('AvoirFournisseur', { skip: skipStatsData });
  const { data: avoirsComptant = [], isLoading: avoirsComptantLoading } = useGetBonsByTypeQuery('AvoirComptant', { skip: skipStatsData });
  const { data: avoirsEcommerce = [], isLoading: avoirsEcommerceLoading } = useGetBonsByTypeQuery('AvoirEcommerce', { skip: skipStatsData });
  const isStatsDataLoading = isPasswordVerified && (
    productsLoading ||
    clientsLoading ||
    bonsSortieLoading ||
    bonsComptantLoading ||
    bonsEcommerceLoading ||
    bonsCommandesLoading ||
    avoirsClientLoading ||
    avoirsFournisseurLoading ||
    avoirsComptantLoading ||
    avoirsEcommerceLoading
  );
  ...
  const clientById = useMemo(() => {
    ...
    for (const client of clients as any[]) {
      if (client?.id != null) map.set(String(client.id), client);
    }
    return map;
  }, [clients]);
  ...
  const { productClientStats, clientProductStats } = useMemo(() => {
    if (!isPasswordVerified) return { productClientStats: {}, clientProductStats: {} };
    ...
    for (const bon of clientBonsForItems) {
      ...
      for (const it of items) {
        ...
        const productData: any = productById.get(String(productId));
  ...
  }, [clientBonsForItems, useClientCondition, productById, isPasswordVerified]);

  // Options recherchables (produits & clients)
  const productOptions = useMemo(() => {
    if (!isPasswordVerified) return [{ value: "", label: "Tous" }];
    ...
    const mapped = Array.from(idSet).map((pid) => {
      const p: any = productById.get(String(pid));
  ...
  }, [productClientStats, products, productById, isPasswordVerified]);
  ...
  const clientOptions = useMemo(() => {
    ...
    const mapped = ids.map((cid) => {
      ...
      const c: any = clientById.get(String(cid));
      const label = c?.nom_complet ? String(c.nom_complet) : `Client ${cid}`;
      return { value: cid, label };
    });
    return base.concat(mapped);
  }, [clientProductStats, clientById, isPasswordVerified]);
          ...
          <form onSubmit={handlePasswordVerification}>
            ...
            <div className="flex gap-3">
              ...
              <button
                ...
                className="flex-1 bg-blue-600 text-white py-2 px-4 rounded-md hover:bg-blue-700 transition-colors font-medium disabled:opacity-60 disabled:cursor-not-allowed"
              >
                        ...
                        <tbody className="bg-white divide-y divide-gray-200">
                          {clientRows.length === 0 ? (
                            ...
                            if (String(cr.clientId) === '__all__') {
                              ...
                              const c = clientById.get(String(cr.clientId));
            ...
            {(() => {
              ...
              return top.map((row: any) => {
                ...
                if (String(row.clientId) === '__all__') {
                  ...
                  const c = clientById.get(String(row.clientId));
                      ...
                      <table className="w-full divide-y divide-gray-200">
                        ...
                        <tbody className="bg-white divide-y divide-gray-200">
                          {productRows.map((pr: any) => {
                            const p = productById.get(String(pr.productId));
