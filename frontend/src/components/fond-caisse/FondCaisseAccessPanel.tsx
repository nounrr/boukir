...
import {
  ...
  type FondCaissePermissionEmployee,
} from '../../store/api/fondCaisseApi';
...
/**
 * Panneau PDG : désigner les employés autorisés à saisir uniquement le fond
 * initial de la caisse. Ces employés ne voient ni les données, ni les calculs,
 * ni les détails, qui restent gérés par le PDG.
 */
const FondCaisseAccessPanel = () => {
  const { data = [], isLoading, isError, refetch } = useGetFondCaissePermissionsQuery();
  const [updatePermissions] = useUpdateFondCaissePermissionsMutation();
  ...
  const authorizedCount = data.filter((e) => e.ouverture && !e.verrouille).length;

  const toggle = async (employee: FondCaissePermissionEmployee) => {
    if (employee.verrouille || savingId === employee.id) return;
    setSavingId(employee.id);
    try {
      await updatePermissions({ id: employee.id, permissions: { ouverture: !employee.ouverture } }).unwrap();
      showSuccess(
        employee.ouverture
          ? `${displayName(employee)} ne peut plus ouvrir le fond de caisse.`
          : `${displayName(employee)} peut désormais saisir le fond initial.`,
      );
    } catch (error) {
    ...
    <section className="mb-6 overflow-hidden rounded-lg border border-indigo-200 bg-white shadow-sm">
      <div className="flex flex-col gap-3 border-b border-gray-200 bg-indigo-50/40 px-5 py-4 sm:flex-row sm:items-end sm:justify-between">
        <div className="flex items-start gap-3">
          ...
          <div>
            <h2 className="text-sm font-black text-gray-900">Autorisation d’ouverture du fond de caisse</h2>
            <p className="mt-0.5 text-xs text-gray-500">
              L’employé autorisé peut uniquement enregistrer le fond initial de la caisse (montant, mode, date/heure).
              Il ne voit ni les données, ni les calculs, ni les détails : tout reste géré par le PDG.
            </p>
          ...
          <table className="min-w-full text-sm">
            <thead className="sticky top-0 bg-gray-50 text-left text-[11px] font-bold uppercase tracking-wide text-gray-500">
              <tr>
                ...
                <th className="px-4 py-3">Ouverture fond initial</th>
                <th className="px-4 py-3">Données &amp; calculs</th>
              </tr>
            </thead>
                ...
                <tr key={employee.id} className="hover:bg-gray-50">
                  ...
                  <td className="px-4 py-3">
                    <button
                      ...
                      aria-pressed={employee.ouverture}
                      aria-label={`Ouverture du fond de caisse pour ${displayName(employee)}`}
                      className="disabled:cursor-not-allowed disabled:opacity-50"
                    >
                      <span className={`relative block h-6 w-11 rounded-full transition-colors ${employee.ouverture ? 'bg-blue-600' : 'bg-gray-300'}`}>
                        <span className={`absolute top-1 h-4 w-4 rounded-full bg-white shadow transition-transform ${employee.ouverture ? 'translate-x-6' : 'translate-x-1'}`} />
                      </span>
                    </button>
                  ...
                  <td className="px-4 py-3">
                    {employee.gestion ? (
                      <span className="rounded-md bg-emerald-50 px-2 py-1 text-xs font-bold text-emerald-700">PDG · accès complet</span>
                    ) : (
                      <span className="rounded-md bg-gray-100 px-2 py-1 text-xs font-semibold text-gray-500">Masqués</span>
                    )}
                  </td>
                </tr>
              ))}
...
export default FondCaisseAccessPanel;
